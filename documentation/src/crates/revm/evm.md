...
It is consisting of two main parts the `Context` and the `Handler`. `Context` represents the state that is needed for execution and `Handler` contains a list of functions that act as a logic.
